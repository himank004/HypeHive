...
const Navbar = () => {
  ...
  const isDashboardVisible = user?.role === "admin" || user?.role === "seller";

  // Determine dashboard label and link
  const dashboardLabel = user?.role === "admin" ? "Admin Dashboard" : "Seller Dashboard";
  const dashboardLink = user?.role === "admin" ? "/secret-dashboard" : "/seller-dashboard";
  ...
  />
          ...
          <nav className="flex flex-wrap items-center gap-4">
            ...
            {/* Show dashboard only for admin and seller */}
            {isDashboardVisible && (
              <Link
